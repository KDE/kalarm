...
ActionReply RtcWakeAction::settimer(const QVariantMap& args)
{
    ...
    switch (result)
    {
        ...
        case -2:
            ...
            break;
        default:
            errmsg = xi18nc("@text/plain", "Error setting wake from suspend.<nl/>Command was: <command>%1 %2</command><nl/>Error code: %3.", proc.program(), proc.arguments().join(' '_L1), result);
            break;
    }
