...
public:
  ...
  /** Flags for use in D-Bus calls, etc. Flags may be combined by OR'ing them
   * together. */
  enum Flag {
    ...
    CONFIRM_ACK = 0x10,  //!< closing the alarm message window requires a
                         //!< confirmation prompt
    EMAIL_BCC = 0x20,    //!< blind copy the email to the user
    DEFAULT_FONT = 0x40, //!< use the default alarm message font. Overrides any
                         //!< specified font.
    REPEAT_SOUND = 0x80, //!< repeat the sound file while the alarm is displayed
    DISABLED = 0x100,    //!< the alarm is currently disabled
    AUTO_CLOSE =
        0x200, //!< auto-close the alarm window after the late-cancel period
    ...
    EXCL_HOLIDAYS =
        0x4000, //!< don't trigger the alarm on holidays. Only valid
    ...
    REMINDER_ONCE =
        0x20000, //!< only trigger the reminder on the first recurrence
    DONT_SHOW_ERROR = 0x40000, //!< do not notify command alarm errors to user
    NOTIFY = 0x80000 //!< use the standard notification system instead of alarm
                     //!< message window
  ...
  enum Actions {
    ...
    ACT_DISPLAY_COMMAND =
        ACT_DISPLAY | ACT_COMMAND, //!< the alarm displays command output
    ACT_ALL =
        ACT_DISPLAY | ACT_COMMAND | ACT_EMAIL | ACT_AUDIO //!< all types mask
  };
  ...
  enum OccurType {
    ...
    FIRST_OR_ONLY_OCCURRENCE =
        0x01, //!< the first occurrence (takes precedence over LAST_RECURRENCE)
    ...
    OCCURRENCE_REPEAT =
        0x10, //!< (bitmask for a sub-repetition of an occurrence)
    FIRST_OR_ONLY_OCCURRENCE_REPEAT =
        OCCURRENCE_REPEAT |
        FIRST_OR_ONLY_OCCURRENCE, //!< a sub-repetition of the first occurrence
    RECURRENCE_DATE_REPEAT =
        OCCURRENCE_REPEAT |
        RECURRENCE_DATE, //!< a sub-repetition of a date-only recurrence
    RECURRENCE_DATE_TIME_REPEAT =
        OCCURRENCE_REPEAT |
        RECURRENCE_DATE_TIME, //!< a sub-repetition of a date/time recurrence
    LAST_RECURRENCE_REPEAT =
        OCCURRENCE_REPEAT |
        LAST_RECURRENCE //!< a sub-repetition of the last recurrence
  };
  ...
  enum OccurOption {
    ...
    ALLOW_FOR_REPETITION //!< if a sub-repetition is the next occurrence, return
                         //!< the previous recurrence, not the sub-repetition
  };

  /** What type of occurrence currently limits how long the alarm can be
   * deferred. */
  enum DeferLimitType {
  ...
  enum ExtraActionOption {
    ...
    DontShowPreActError =
        0x02, //!< do not notify pre-alarm action errors to user
    ExecPreActOnDeferral =
        0x04 //!< execute pre-alarm action also for deferred alarms
  };
  ...
  enum UidAction {
    ...
    UID_CHECK,  //!< verify that the KCal::Event UID is already the same as the
                //!< KAEvent ID, if the latter is non-empty
  ...
  KAEvent(const KADateTime &dt, const QString &name, const QString &text,
          ...
          SubAction action, int lateCancel, Flags flags,
   ...
   *                             are left untouched.
   */
  bool updateKCalEvent(const KCalendarCore::Event::Ptr &event, UidAction u,
                       bool setCustomProperties = true) const;
  ...
  /** Return whether the event's main alarm has expired. If so, a deferral alarm
   * will exist. */
  ...
  /** Set the event's unique identifier. Note that the UID is guaranteed to be
   * unique only within the calendar containing the event.
   */
  void setEventId(const QString &id);

  /** Return the event's unique identifier. Note that the UID is guaranteed to
   * be unique only within the calendar containing the event.
   */
  QString id() const;

  /** Increment the revision number of the event (SEQUENCE property in
   * iCalendar). */
  ...
  /** Note the event's storage format compatibility compared to the current
   * KAlarm calendar format. */
  void setCompatibility(KACalendar::Compat c);

  /** Return the event's storage format compatibility compared to the current
   * KAlarm calendar format. */
  KACalendar::Compat compatibility() const;

  /** Return the original KCalendarCore::Event's custom properties in the source
   * calendar. */
  ...
  /** Return the OR of the basic action types of the event's main alarm
   * (display, command, email, audio). Note that for a display alarm whose text
   * is generated by a command, the returned type is @c ACT_DISPLAY|ACT_COMMAND.
   */
  ...
  /** Return the path of the file whose contents are to be shown, for a display
   * alarm.
  ...
  /** Return whether to execute the command in a terminal window, for a command
   * alarm. */
  bool commandXterm() const;

  /** Return whether the command output is to be displayed in an alarm message
   * window. */
  bool commandDisplay() const;

  /** Set or clear the command execution error for the last time the alarm
   * triggered. */
  ...
  /** Return whether execution errors for the command should not to be shown to
   * the user.
   *  @since 20.08
   */
  ...
  /** Return whether alarm acknowledgement must be confirmed by the user, for a
   * display alarm. */
  ...
  void setEmail(uint from, const KCalendarCore::Person::List &,
  ...
  /** Return a string containing the email addressees, including names, for an
   * email alarm.
   ...
   */
  static QString
  joinEmailAddresses(const KCalendarCore::Person::List &addresses,
                     const QString &sep);
  /** Return the list of email addressees, excluding names, for an email alarm.
   */
  QStringList emailPureAddresses() const;

  /** Return a string containing the email addressees, excluding names, for an
   * email alarm.
   *  @param sep  separator string to insert between addresses.
   */
  ...
  /** Return whether to send a blind copy of the email to the sender, for an
   * email alarm. */
  bool emailBcc() const;
   ...
   *  @param fadeSeconds    number of seconds to fade from @p fadeVolume to @p
   * volume
   *  @param repeatPause    number of seconds to pause between repetitions, or
   * -1 if no repeat
   *  @param allowEmptyFile true to set the volume levels even if @p filename is
   * empty
   *  @see audioFile(), soundVolume(), fadeVolume(), fadeSeconds()
   */
  void setAudioFile(const QString &filename, float volume, float fadeVolume,
                    int fadeSeconds, int repeatPause = -1,
   ...
   *  @param afterTime number of minutes after default time to schedule alarm
   * for, or -1 to not use 'time from now'
   *  @see isTemplate(), name()
   */
   ...
   *  @return @c true if no time is specified, i.e. the normal default alarm
   * time will be used, @c false if the template specifies a time.
   */
   ...
   *  this template will have the "Time from now" radio button enabled in the
   * alarm edit dialog.
   *  @return minutes after the default time, or -1 if the template specifies a
   * time of day or a date-only alarm.
   */
  ...
  void setActions(const QString &pre, const QString &post,
                  ExtraActionOptions preOptions);

  /** Return the shell command to execute before the alarm is displayed. */
  QString preAction() const;

  /** Return the shell command to execute after the display alarm is
   * acknowledged.
   *  @see setActions()
   */
   ...
   *  @param minutes  number of minutes BEFORE the main alarm; if negative, the
   * reminder will occur AFTER the main alarm. 0 = clear the reminder.
   ...
   *  activate the event's reminder which occurs after the given main alarm
   * time. If there is no reminder after the main alarm, this method does
   * nothing.
   *  @return @c true if successful (i.e. reminder falls before the next main
   * alarm).
   */
  void activateReminderAfter(const DateTime &mainAlarmTime);

  /** Return the number of minutes BEFORE the main alarm when a reminder alarm
   * is set.
  ...
  /** Return whether a reminder is currently due (before the next, or after the
   * last, main alarm/recurrence).
   *  @see reminderDeferral()
   */
  bool reminderActive() const;

  /** Return whether the reminder alarm is triggered only for the first
   * recurrence.
   *  @see setReminder()
   */
   ...
   *  @param adjustRecurrence if true, ensure that the next scheduled recurrence
  ...
  /** Return the time at which the currently pending deferred alarm should
   * trigger.
   ...
   *  @param limitType  if non-null, pointer to variable which will be updated
   * to hold the type of occurrence which currently limits the deferral.
   ...
   *
   *  @param withRepeats  true to include sub-repetitions, false to exclude
   * them.
   ...
   *  @param events list of events. Any date-time events in the list are
   * ignored.
   *  @see setStartOfDay()
   */
  ...
  /** Return the date/time the event was created, or saved in the archive
   * calendar.
   ...
   *  @param includeArchived  true to also test for archived repeat-at-login
   * status, false to test only for a current repeat-at-login alarm.
  ...
  /** Return whether the alarm is disabled on non-working days and outside
   * working hours.
   *  @see setWorkTimeOnly()
   */
   ...
   *  @param days      bits set to 1 for each working day. Array element 0 =
   * Monday ... 6 = Sunday.
   *  @param start     start time in working day.
   *  @param end       end time in working day.
   *  @param timeSpec  time spec to be used for evaluating the start and end of
   * the working day.
   *  @see setWorkTimeOnly(), excludedByWorkTimeOrHoliday()
   */
  static void setWorkTime(const QBitArray &days, const QTime &start,
                          const QTime &end, const KADateTime::Spec &timeSpec);
   ...
   *  @see setRecurMinutely(), setRecurDaily(), setRecurWeekly(),
   * setRecurMonthlyByDate(), setRecurMonthlyByPos(), setRecurAnnualByDate(),
   * setRecurAnnualByPos(), setFirstRecurrence()
   */
  ...
  /** Set the recurrence to recur monthly, on the specified days within the
   * month.
   ...
   *  @return @c false if no recurrence was set up.
   */
  bool setRecurMonthlyByDate(int freq, const QList<int> &days, int count,
                             QDate end);
  ...
  struct MonthPos {
     ...
     *  @return @c false if no recurrence was set up.
     */
    bool setRecurMonthlyByPos(int freq, const QList<MonthPos> &pos, int count,
                              QDate end);
     ...
     *  @return @c false if no recurrence was set up.
     */
    bool setRecurAnnualByDate(int freq, const QList<int> &months, int day,
                              KARecurrence::Feb29Type, int count, QDate end);
     ...
     *  @return @c false if no recurrence was set up.
     */
    bool setRecurAnnualByPos(int freq, const QList<MonthPos> &pos,
