...
void DirResourceImportDialog::pageChanged(KPageWidgetItem* current, KPageWidgetItem* before)
{
    ...
    if (current)
    {
        DirResourceImportWidgetBase* page = static_cast<DirResourceImportWidgetBase*>(current->widget());
        page->setTextSizes();
        DirResourceImportTypeWidget* typePage = qobject_cast<DirResourceImportTypeWidget*>(page);
        if (typePage)
            typePage->validate();
    }
}
...
{
    DirResourceImportTypeWidget* page = qobject_cast<DirResourceImportTypeWidget*>(currentPage()->widget());
    if (page)
    {
