...
//	KAResourceRemote* res = dynamic_cast<KAResourceRemote*>(resource);
	...
	if (!res)
		kError(KARES_DEBUG) << "ResourceRemoteConfigWidget::loadSettings(KAResourceRemote): cast failed" << endl;
	else
	{
...
#ifndef NDEBUG
		kDebug(KARES_DEBUG) << "ResourceRemoteConfigWidget::loadSettings(): File " << mDownloadUrl->url() << " type " << res->typeName() << endl;
#endif
	}
}
